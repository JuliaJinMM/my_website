...
We explore whether the mean IMDB rating for Steven Spielberg and Tim Burton are the same or not. The confidence intervals for the mean ratings of these two directors overlap. In addition, we will run a hypothesis test, using use both the `t.test` command and the `infer` package to simulate from a null distribution, assuming zero difference between the two.
